...
static void lkcbfunc(pmix_status_t status, void *cbdata)
{
    ...
    OPAL_POST_OBJECT(lk);
    lk->status = pmix4x_convert_rc(status);
    OPAL_PMIX_WAKEUP_THREAD(lk);
}

int pmix4x_server_init(opal_pmix_server_module_t *module,
                      opal_list_t *info)
{
    pmix_status_t rc;
    int dbg;
    ...
    PMIx_Register_event_handler(NULL, 0, pinfo, 1, pmix4x_event_hdlr, errreg_cbfunc, (void*)event);
...
int pmix4x_server_finalize(void)
{
    pmix_status_t rc;
...
int pmix4x_server_gen_regex(const char *input, char **regex)
{
    pmix_status_t rc;
...
int pmix4x_server_gen_ppn(const char *input, char **ppn)
{
    pmix_status_t rc;
...
int pmix4x_server_register_nspace(opal_jobid_t jobid,
                                 int nlocalprocs,
                                 opal_list_t *info,
            ...
            if (0 == strcmp(kv->key, OPAL_PMIX_PROC_DATA)) {
                ...
                if (0 < szmap) {
                    ...
                    OPAL_LIST_FOREACH(k2, pmapinfo, opal_value_t) {
                        ...
                        pmix4x_value_load(&pmap[m].value, k2);
                        ++m;
                    }
                }
...
void pmix4x_server_deregister_nspace(opal_jobid_t jobid,
                                     opal_pmix_op_cbfunc_t cbfunc,
                                     void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(jptr, &mca_pmix_pmix4x_component.jobids, opal_pmix4x_jobid_trkr_t) {
        if (jptr->jobid == jobid) {
            ...
            OPAL_PMIX_ACQUIRE_THREAD(&opal_pmix_base.lock);
            opal_list_remove_item(&mca_pmix_pmix4x_component.jobids, &jptr->super);
            OBJ_RELEASE(jptr);
            break;
        }
    }
...
int pmix4x_server_register_client(const opal_process_name_t *proc,
                                 uid_t uid, gid_t gid,
                                 void *server_object,
                                 opal_pmix_op_cbfunc_t cbfunc,
                                 void *cbdata)
 ...
 * done executing */
void pmix4x_server_deregister_client(const opal_process_name_t *proc,
                                     opal_pmix_op_cbfunc_t cbfunc,
                                     void *cbdata)
{
    opal_pmix4x_jobid_trkr_t *jptr;
...
int pmix4x_server_setup_fork(const opal_process_name_t *proc, char ***env)
{
    pmix_status_t rc;
    pmix_proc_t p;
...
int pmix4x_server_dmodex(const opal_process_name_t *proc,
...
int pmix4x_server_notify_event(int status,
                               ...
                               opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == source) {
        ...
        op->p.rank = pmix4x_convert_opalrank(OPAL_VPID_INVALID);
    } else {
        (void)opal_snprintf_jobid(op->p.nspace, PMIX_MAX_NSLEN, source->jobid);
        op->p.rank = pmix4x_convert_opalrank(source->vpid);
    }
...
int pmix4x_server_iof_push(const opal_process_name_t *source,
...
{
    pmix4x_opcaddy_t *op = (pmix4x_opcaddy_t*)provided_cbdata;
...
int pmix4x_server_setup_application(opal_jobid_t jobid,
                                    opal_list_t *info,
...
int pmix4x_server_setup_local_support(opal_jobid_t jobid,
                                      opal_list_t *info,
                                      opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    opal_value_t *kv;
