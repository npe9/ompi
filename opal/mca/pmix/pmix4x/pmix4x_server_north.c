...
{
    pmix4x_opalcaddy_t *opalcaddy = (pmix4x_opalcaddy_t*)cbdata;

    OPAL_ACQUIRE_OBJECT(opalcaddy);
    if (NULL != opalcaddy->opcbfunc) {
        opalcaddy->opcbfunc(pmix4x_convert_opalrc(status), opalcaddy->cbdata);
    }
                                                ...
                                                pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    pmix4x_opalcaddy_t *opalcaddy;

    if (NULL == host_module || NULL == host_module->client_connected) {
        return PMIX_SUCCESS;
    }

    opalcaddy = OBJ_NEW(pmix4x_opalcaddy_t);
    ...
    proc.vpid = pmix4x_convert_rank(p->rank);
                                     ...
                                     pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    for (n=0; n < nprocs; n++) {
        ...
        nm->name.vpid = pmix4x_convert_rank(procs[n].rank);
    }
                           ...
                           opal_pmix_release_cbfunc_t relcbfunc, void *relcbdata)
{
    ...
    if (opal_pmix_collect_all_data) {
        ...
        while (NULL != (dmdx = (opal_pmix4x_dmx_trkr_t*)opal_list_remove_first(&mca_pmix_pmix4x_component.dmdx))) {
                                          ...
                                          pmix_modex_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (opal_pmix_base_async_modex && opal_pmix_collect_all_data) {
        OPAL_PMIX_ACQUIRE_THREAD(&opal_pmix_base.lock);
        dmdx = OBJ_NEW(opal_pmix4x_dmx_trkr_t);
        dmdx->cbfunc = cbfunc;
        dmdx->cbdata = cbdata;
        opal_list_append(&mca_pmix_pmix4x_component.dmdx, &dmdx->super);
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
        return PMIX_SUCCESS;
    }
                                       ...
                                       pmix_op_cbfunc_t cbfunc, void *cbdata)
{
   ...
   /* convert the nspace/rank to an opal_process_name_t */
    ...
    for (n=0; n < ninfo; n++) {
        ...
        if (OPAL_SUCCESS != (rc = pmix4x_value_unload(oinfo, &info[n].value))) {
            OBJ_RELEASE(opalcaddy);
            return pmix4x_convert_opalrc(rc);
        }
    }
    ...
    if (NULL != opalcaddy->lkupcbfunc) {
        ...
        if (NULL != data) {
            ...
            OPAL_LIST_FOREACH(p, data, opal_pmix_pdata_t) {
                ...
                d[n].proc.rank = pmix4x_convert_opalrank(p->proc.vpid);
                (void)strncpy(d[n].key, p->value.key, PMIX_MAX_KEYLEN);
                pmix4x_value_load(&d[n].value, &p->value);
            }
        }
                                     ...
                                     pmix_spawn_cbfunc_t cbfunc, void *cbdata)
{
    ...
    for (k=0; k < ninfo; k++) {
        ...
        if (OPAL_SUCCESS != (rc = pmix4x_value_unload(oinfo, &job_info[k].value))) {
            OBJ_RELEASE(opalcaddy);
            return pmix4x_convert_opalrc(rc);
        }
    }
    ...
    for (n=0; n < napps; n++) {
        ...
        for (k=0; k < apps[n].ninfo; k++) {
            ...
            if (OPAL_SUCCESS != (rc = pmix4x_value_unload(oinfo, &apps[n].info[k].value))) {
                OBJ_RELEASE(opalcaddy);
                return pmix4x_convert_opalrc(rc);
            }
        }
                                         ...
                                         pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    src.vpid = pmix4x_convert_rank(source->rank);
...
{
    pmix4x_opcaddy_t *pcaddy = (pmix4x_opcaddy_t*)cbdata;
...
{
    pmix4x_opalcaddy_t *opalcaddy = (pmix4x_opalcaddy_t*)cbdata;
    pmix4x_opcaddy_t *pcaddy;
    opal_value_t *kv;
    size_t n;

    pcaddy = OBJ_NEW(pmix4x_opcaddy_t);

    /* convert the status */
    pcaddy->status = pmix4x_convert_opalrc(status);
    ...
    if (NULL != info && 0 < (pcaddy->ninfo = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(kv, info, opal_value_t) {
            ...
            pmix4x_value_load(&pcaddy->info[n].value, kv);
            ++n;
        }
    }
                                  ...
                                  void *cbdata)
{
    ...
    requestor.vpid = pmix4x_convert_rank(proct->rank);
    ...
    for (n=0; n < nqueries; n++) {
        ...
        for (m=0; m < queries[n].nqual; m++) {
            ...
            if (0 == strcmp(queries[n].qualifiers[m].key, PMIX_NSPACE)) {
                ...
                if (OPAL_SUCCESS != (rc = pmix4x_value_unload(oinfo, &queries[n].qualifiers[m].value))) {
                    OBJ_RELEASE(opalcaddy);
                    return pmix4x_convert_opalrc(rc);
                }
            }
                                   ...
                                   void *cbdata)
{
    ...
    for (n=0; n < ninfo; n++) {
        ...
        } else if (OPAL_SUCCESS != (rc = pmix4x_value_unload(oinfo, &info[n].value))) {
            OBJ_RELEASE(opalcaddy);
            err = pmix4x_convert_opalrc(rc);
            if (NULL != cbfunc) {
                cbfunc(err, NULL, cbdata);
                       ...
                       pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    for (n=0; n < ndata; n++) {
         ...
         * server function both agree on what will be there */
        opal_list_append(&opalcaddy->info, &oinfo->super);
        if (OPAL_SUCCESS != (rc = pmix4x_value_unload(oinfo, &data[n].value))) {
            OBJ_RELEASE(opalcaddy);
            ret = pmix4x_convert_opalrc(rc);
            if (NULL != cbfunc) {
                cbfunc(ret, cbdata);
    ...
    for (n=0; n < ndirs; n++) {
        ...
        if (OPAL_SUCCESS != (rc = pmix4x_value_unload(oinfo, &directives[n].value))) {
            OBJ_RELEASE(opalcaddy);
            ret = pmix4x_convert_opalrc(rc);
            if (NULL != cbfunc) {
                cbfunc(ret, cbdata);
                                     ...
                                     pmix_info_cbfunc_t cbfunc, void *cbdata)
{
    ...
    odir = pmix4x_convert_allocdir(directive);
                                        ...
                                        pmix_info_cbfunc_t cbfunc, void *cbdata)
{
    ...
    for (n=0; n < ntargets; n++) {
        ...
        nm->name.vpid = pmix4x_convert_rank(targets[n].rank);
    }
